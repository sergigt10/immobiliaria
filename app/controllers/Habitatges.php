    ...
    public function add(){
      ...
      if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $data = [
          ...
          'pdf_1' => trim($_POST['pdf_1']),
          'pdf_2' => trim($_POST['pdf_2']),
          'valor_consum' => trim($_POST['valor_consum']),
          'valor_emisio' => trim($_POST['valor_emisio']),
          'portada' => (!isLoggedInAndAdmin()) ? '0' : trim($_POST['portada']),
          'preu' => trim($_POST['preu']),
          'index_lloguer' => trim($_POST['index_lloguer']),
          'preu_comunitat' => trim($_POST['preu_comunitat']),
          'preu_contribucio' => trim($_POST['preu_contribucio']),
          'preu_basura' => trim($_POST['preu_basura']),
          ...
          'consum_id' => trim($_POST['consum_id']),
          'emisio_id' => trim($_POST['emisio_id']),
        ...
        // Make sure no errors
        if(empty($data['titol_cat_err']) && empty($data['titol_esp_err']) && empty($data['referencia_err'])){
          // Validated
          ...
          if (!empty($data['pdf_1'])) {
            ...
            {        
              ...
              if ($tamano_archivo >= $volumen_min AND $tamano_archivo <= $volumen_max AND ($tipo_archivo == $archivo_permitido1)){
                $id_thumb=rand(1, 50);
                $new_nombre = "1-".$data['slug_esp']."-".$id_thumb."-".$data['usuari_id']."-".uniqid().".".$ext;
    ...
    public function edit($id){
      ...
      if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $data = [
          ...
          'consum_id' => trim($_POST['consum_id']),
          'emisio_id' => trim($_POST['emisio_id']),
          'usuari_id' => (!isLoggedInAndAdmin()) ? $_SESSION['usuari_id'] : $immoble->usuari_id,
          'titol_cat_err' => '',
          'titol_esp_err' => '',
          'pdf_1_err' => '',
          'pdf_2_err' => '',
        ...
        if(empty($data['titol_cat_err']) && empty($data['titol_esp_err']) && empty($data['referencia_err'])){
          ...
          $dataImg = [
            ...
            'pdf_1' => $getimmobleImg->pdf_1,
            'pdf_2' => $getimmobleImg->pdf_2
          ];
        ...
        $data = [
          ...
          'pdf_1' => $immoble->pdf_1,
          'pdf_2' => $immoble->pdf_2,
          'valor_consum' => $immoble->valor_consum, 
          'valor_emisio' => $immoble->valor_emisio,
          'portada' => $immoble->portada,
          'preu' => $immoble->preu,
          'index_lloguer' => $immoble->index_lloguer,
          'preu_comunitat' => $immoble->preu_comunitat,
          'preu_contribucio' => $immoble->preu_contribucio,
          'preu_basura' => $immoble->preu_basura,
          ...
          'consum_id' => $immoble->consum_id,
          'emisio_id' => $immoble->emisio_id,
          'provincies' => $provincies,
          'idProvinciaByPoblacio' => $idProvinciaByPoblacio->provincia_id,
          'poblacions' => $poblacions,
          'caracteristiques' => $caracteristiques,
